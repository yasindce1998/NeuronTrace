...
pub fn get_current_generation(bpf: &Ebpf) -> Result<u32> {
    let gen_map: Array<_, GenerationCounter> =
        Array::try_from(bpf.map("GENERATION").context("GENERATION map not found")?)?;
...
pub fn set_generation(bpf: &mut Ebpf, value: u32) -> Result<()> {
    let mut gen_map: Array<_, GenerationCounter> =
        Array::try_from(bpf.map_mut("GENERATION").context("GENERATION map not found")?)?;
