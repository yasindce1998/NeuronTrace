...
pub fn handle_exec(_ctx: &LsmContext) -> Result<i32, i64> {
    ...
    if !check_generation(pid) {
        emit_event(pid, tgid, EventType::Exec, PolicyAction::Block);
        return Ok(-1); // -EPERM
    }
    ...
    match action {
        ...
        PolicyAction::Kill => {
            emit_event(pid, tgid, EventType::Exec, PolicyAction::Kill);
            Ok(-9) // will be caught by userspace to send SIGKILL
        }
        PolicyAction::Audit => {
            emit_event(pid, tgid, EventType::Exec, PolicyAction::Audit);
            Ok(0) // allow but log
        }
    }
}
