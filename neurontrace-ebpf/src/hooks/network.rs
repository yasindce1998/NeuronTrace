...
pub fn handle_connect(_ctx: &LsmContext) -> Result<i32, i64> {
    ...
    if !check_generation(pid) {
        emit_event(pid, tgid, EventType::Connect, PolicyAction::Block);
        return Ok(-1);
    }
    ...
    match action {
        ...
        PolicyAction::Kill => {
            emit_event(pid, tgid, EventType::Connect, PolicyAction::Kill);
            Ok(-9)
        }
        PolicyAction::Audit => {
            emit_event(pid, tgid, EventType::Connect, PolicyAction::Audit);
            Ok(0)
        }
    }
}
