...
pub fn handle_file_open(ctx: &LsmContext) -> Result<i32, i64> {
    ...
    if !check_generation(pid) {
        emit_event(pid, tgid, EventType::Open, PolicyAction::Block);
        return Ok(-1);
    }
    ...
    match action {
        ...
        PolicyAction::Kill => {
            emit_event(pid, tgid, EventType::Open, PolicyAction::Kill);
            Ok(-9)
        }
        PolicyAction::Audit => {
            emit_event(pid, tgid, EventType::Open, PolicyAction::Audit);
            Ok(0)
        }
    }
}
...
pub fn handle_unlink(ctx: &LsmContext) -> Result<i32, i64> {
    ...
    match action {
        PolicyAction::Allow => Ok(0),
        PolicyAction::Block => {
            emit_event(pid, tgid, EventType::Unlink, PolicyAction::Block);
            Ok(-1)
        }
        PolicyAction::Kill => {
            emit_event(pid, tgid, EventType::Unlink, PolicyAction::Kill);
            Ok(-9)
        }
        PolicyAction::Audit => {
            emit_event(pid, tgid, EventType::Unlink, PolicyAction::Audit);
            Ok(0)
        }
    }
}
...
pub fn handle_rename(ctx: &LsmContext) -> Result<i32, i64> {
    ...
    match action {
        PolicyAction::Allow => Ok(0),
        PolicyAction::Block => {
            emit_event(pid, tgid, EventType::Rename, PolicyAction::Block);
            Ok(-1)
        }
        PolicyAction::Kill => {
            emit_event(pid, tgid, EventType::Rename, PolicyAction::Kill);
            Ok(-9)
        }
        PolicyAction::Audit => {
            emit_event(pid, tgid, EventType::Rename, PolicyAction::Audit);
            Ok(0)
        }
    }
}
